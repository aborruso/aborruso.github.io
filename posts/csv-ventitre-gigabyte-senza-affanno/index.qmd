...
La **novità più importante** di questo aggiornamento, è lagata alla **natura degli interventi in elenco**: a "Lavori Pubblici" e "Incentivi alle imprese e Contributi per calamità naturali", si aggiungono "acquisti di beni, servizi, corsi di formazione, strumenti finanziari, progetti di ricerca e contributi a entità diverse dalle unità produttive per una Pubblica Amministrazione più trasparente e vicina al cittadino" (qui la [notizia](https://www.opencup.gov.it/portale/web/opencup/home/-/asset_publisher/zIMyNGpUy0tF/content/14-febbraio-2024-il-nuovo-portale-opencup) di lancio).<br>
...
Bisogna scorre la pagina e guardare la sezione "**Progetti OpenData**".

In questo post, mosterò rapidamente come esplorare il [file "Complessivo"](https://www.opencup.gov.it/portale/web/opencup/dettaglio-opendata-complessivo) di tutti i progetti.

È un **file "grande"**. Quello in formato `CSV` è un che compresso pesa circa **2.9 GB**, e **decompresso 23 GB**. È probabilmente **il file CSV più grande** (o uno dei più grandi), pubblicato in una sezione open data di un sito di una **Pubblica Amministrazione italiana**.

Non è gestibile con un comune foglio di calcolo, né con tanti altri strumenti.<br>
A riga di comando invece si può ossevare, filtrare, e analizzare, con grande leggerezza e rapidità.

## La prima lettura

Un file così grande richiede un po' di tempo per il download e poi per la decopressione.<br>
...
L'*utility* **`grep`** - una delle più importanti di tutti i tempio è perfetta per questo scopo.
...
È utilissima per capire per cosa è possibile usare questi dati, quali sono le informazioni che ci interessano di più, che storia poter raccontare, che mappa creare, che *dashboard* realizzare, come usare l'integgligenza artificiale per arricchirli, come metterli in relazione con altri dati, ecc..
