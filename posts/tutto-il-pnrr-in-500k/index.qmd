...
L'ispirazione per la creazione di questo modalità di accesso ai dati del PNRR, viene dalla lettura di [questo ottimo articolo](https://motherduck.com/blog/from-data-lake-to-lakehouse-can-duckdb-be-the-best-portable-data-catalog/), in cui si spiega come DuckDB possa diventare un **catalogo portatile** per la **gestione** dei **dati**, da fonti diverse (file `parquet` da fonti Amazon S3, database PostgreSQL, tabelle su Google Cloud, ecc.).
...
- il formato **`CSV`** **non** è **ottimizzato** per ***query* su file "grandi"**, che **non** risiadono sul **proprio** **PC**, perché è necessario scaricare tutto il file per fare la *query*.
