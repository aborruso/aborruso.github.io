...
Uno è quello di **renderlo subito pronto all'uso**, come se fosse già decompresso. Che è una cosa molto comoda specie nelle prime **fasi** di lavoro "**esplorative**", in cui si fanno un po' di "ispeziono", le prime prove di analisi, di verifica qualità, di adeguatezza dei dati, ecc..
...
Per continuare l'esplorazione di questo dataset, è utile ad esempio avere restituito il conteggio dei record, i valori nulli di ogni campo e anche i valori distinti.<br>
...
: *Output* di Miller {#tbl-mlr-output .striped .responsive .small}
...
Nella sezione sui [dati di OpenCoesione](#dati-opencoesione) (leggila prima di proseguire) ho descritto di come sia **necessario** **descrivere** il file **`CSV`** usato come esempio, per poterlo leggere al meglio con qualsiasi applicazione. Questo perché ha alcune problematiche e perché non è appunto "standard".

Fatto lo sforzo di descriverlo, è possibile usare **DuckDB** per **convertire** un **`CSV`** "**brutto, grosso e cattivo**", in un **`CSV`** molto più **pronto all'uso**.

::: {.callout-note}
## Nota bene
Anche un CSV creato in questo modo può dare qualche problema, perché i programmi di analisi - con un file `CSV` - sono costretti a fare l'inferenza dei contenuti. E l'inferenza può essere errata.<br>
Qui inoltre siamo di fronte a un file particolarmente "ricco" e "variegato".
:::

Il comando da usare è in modo schematico `COPY INPUT TO OUTPUT` (documentazione [`COPY`](https://duckdb.org/docs/archive/0.8.1/sql/statements/copy)):
...
Così facendo, il `CSV` di output avrà queste caratteristice:
...
Molti dati pubblicati nei **portali Open Data italiani** in questo fomato sono accompagnati **soltanto** da un **titolo** e una **descrizione**. È un **vuoto informativo** e molto spesso una **barriera** a un pieno uso dei dati.<br>
Alle volte, come nel caso di [OpenCoesione](#dati-opencoesione), è presente un file con i nomi dei campi, la loro tipologia e tante altre note. Nella gran parte dei casi sono però file descrittivi leggibili soltanto a schermo da chi dovrà analizzare i file. Non sono leggibili da un *personal computer* o da un'applicazione, **non sono *machine readable***.<br>

E quando si hanno dati ricchi e complessi, questa diventa un'**ulteriore barriera** all'**uso dei dati**.

::: {.callout-important}
## 📌 Importante
La **pubblicazione** non si dovrebbe limitare a rendere disponibile dei file, ma essere **accompagnata da una descrizione del file stesso**.
:::

Farlo in una delle due modalità descritte sopra sarebbe perfetto. Non è inizialmente a costo zero, ma se è uno dei processi ("automatici") di un progetto di pubblicazione, è un costo che si può sostenere, e che si ripaga in termini di **usabilità** e **riusabilità** dei dati.

Una "**buona pratica**" per chi pubblica dati, basata su uno "***standard de facto***", potrebbe essere quella di accompagnare al file `CSV` anche la *query* `SQL` per la creazione della tabella.<br>
Sotto l'esempio per la creazione della tabella dei progetti di OpenCoesione descritta [qui](#dati-opencoesione).
...
Perché il **`CSV`** è principalemente un **formato di scambio**, e **poterlo** **importare** subito in un *database*** dà una marcia in più.

## Parquet

Parquet è un **formato** di file ***open source*** progettato per l'**archiviazione** e l'**analisi** di **grandi dataset**.<br>
...
- [ ] compresso
- [ ] performance
- [ ] articolo su nuovo formato <https://www-icem7-fr.translate.goog/outils/parquet-devrait-remplacer-le-format-csv/?_x_tr_sl=auto&_x_tr_tl=it&_x_tr_hl=en-US>
- [ ] può essere il futuro, ma non c'è traccia nelle nuove linee guida
- [ ] esempi di parquet e geoparquet nel mondo
- [ ] https://www.robinlinacre.com/parquet_api/
- [ ] https://towardsdatascience.com/a-parquet-file-is-all-you-need-962df86886bb
...
- quante vendite sono state fatte il 2 genaio 2023?
...
Perché sono un vantaggio? Nella *query* di esempio di sopra, non solo bastano due colonne, ma il `Gruppo 2` di righe è inutile (non c'è la "Parmigiana", vedi @fig-row-groups-skip). Con questa struttura a gruppi di righe (qui semplificata per fini didattici) sarà possibile non tenere conto del "Gruppo 2" e la *query* sarà più rapida.

![Per la *query* di esempio, il "Gruppo 2" è inutile](images/row-groups-skip.png){#fig-row-groups-skip width=100%}

### Convertire il file di OpenCoeseione in Parquet

Per convertire il [file di OpenCoesione](#dati-opencoesione) in formato `Parquet` si può usare ancora una volta DuckDB.<br>
Il comando richiederà ancora una volta una ricca descrizione del file di *input*, e di base sarà `COPY INPUT TO OUTPUT` (documentazione [`COPY`](https://duckdb.org/docs/archive/0.8.1/sql/statements/copy)):
...
Il formato `parquet` - accopiato a *client* come [DuckDB](#duckdb) - rende **facile** e **universale** l'accesso ai dati. Questo perché, una volta che un file è pubblico sul web (in `HTTPS`, come `S3`, ecc.), si ha a disposizione un'**intefaccia `SQL`** per interrogarlo, in **modalità `streaming`**; ovvero rendendo disponibili solo le porzioni di dati necessarie per rispondere alle *query*, invece che tutto il file.

È un po' come **avere delle API**, ma senza doverle creare. E da utente senza dovere studiare una nuova documentazione di API, perché basta l'"universale" `SQL` e accedere al file `parquet` dal linguaggio di *scripting* e/o *client* preferiti.
...
Voglio sapere per ogni Missione, il numero dei progetti e il valore del finanziamento PNRR (in euro). Usando come client DuckDB, e abilitando l'estensione `httpfs` (vedi sezione [estensioni](#le-estensioni)), basta lanciare questa *query*:
...
E posso farlo anche da una [pagina web](https://shell.duckdb.org/) che include DuckDb come WebAssembly (vedi @fig-duckdb-wasm).
...
Il formato `parquet` - per alcune dell sue caratteristiche - **potrebbe** **affiancare** (e alla lunga **rimpiazzare**?) il **`CSV`**, come formato di **pubblicazione** di **dati aperti**.<br>
...
DuckDB è un database relazionale *embedded* e *open source* leggero e veloce, di facilissima installazione e gestione.<br>
...
### Faciltà di installazione e utilizzo

La [pagina dedicata](https://duckdb.org/docs/installation/index) è molto comoda:

1. si sceglie la versione;
2. l'ambiente e le modalità di lavoro (io lo utilizzo soprattuto a riga di comando, come `CLI`);
3. la piattaforma;
4. vengono restituite le istruzioni per l'installazione.

E in pochissimi tempo si è pronti all'uso.
...
DuckBD ha delle [**estensioni**](https://duckdb.org/docs/archive/0.8.1/extensions/overview) che ne estendono in modo molto interessante l'utilizzo.

Tre che ho trovato subito utilissime sono:

- [`httpfs`](https://duckdb.org/docs/extensions/httpfs), per leggere file `CSV` e `parquet` direttamente da indirizzi `HTTP(S)` (è [come avere delle API](#come-avere-delle-api)) o tramite `S3`;
...
E con l'estensione `httpfs` e con dati CSV pubblicati in [modo "standard"](#csv-standard) (come quelli sulla COVID-19 pubblicatu dalla Protezione Civile), è ad esempio possibile avere restituito comodamente e rapidamente la media mobile settimanale dei nuovi casi COVID-19 per regione, puntando semplcimente all'URL di un file statico (DuckBD supporta una sintassi `SQL`, dove il `FROM` può essere inserito all'inizio):
...
Alcuni esempi che mi hanno fatto contento mentre scrivevo `query`.
...
```{.sql .code-overflow-wrap}
-- usando la stringa di connessiona vuota, di default
...
E DuckBD può nativamente interrogare Pandas DataFrames, Polars DataFrames e tabelle Arrow.
...
- [x] il SQL figo e comodo (group by all, exclude)
- [ ] json, parquet
- [x] https
- [x] geo
- [ ] integrabile con python e R, ...
- [ ] legge sqlite, csv, postgis, ...
- [ ] spunti
  - https://duckdb.org/docs/data/csv/overview.html
  - https://motherduck.com/blog/exploring-stackoverflow-with-duckdb-on-motherduck-1/
  - https://duckdb.org/docs/data/parquet/tips.html#tips-for-writing-parquet-files
...
Il dataset **Progetti con tracciato esteso** è un file `CSV` compresso in formato `ZIP` che pesa circa 200 MB; decompresso circa 4 GB e mezzo.
...
Lo fa in 0.003 secondi - instantaneamente - perché il file non viene decompresso per intero, ma solo la parte che serve per mostrare le prime 10 righe.
...
E modificando di poco il comando di sopra, con  `unzip -p progetti_esteso_20230430.zip | wc -l`, ho contezza che il file è composto da 1.936.840 righe.

Fatta questa prima esplorazione grezza, di solito voglio "guardare" in modo più "comodo" la tabella dei dati. Potrei salvare le prime 10 righe in un file, aggiungendo `> prime_dieci_righe.csv` al comando di sopra, e infine aprirlo con un foglio elettronico, ma io preferisco usare lo [straordinario **VisiData**](https://ondata.github.io/guidaVisiData/) per farlo.<br>
Con `unzip -p progetti_esteso_20230430.zip | head | vd -f csv --csv-delimiter=";"` ad esempio visualizzo le 10 righe di *output* e tutte le colonne (constato che sono 199), ma soprattutto mi faccio una prima idea dei contenuti.

![Esempio visualizzazione con VisiData](./images/visidata_explore.png){#fig-duckdb-preview-vd}

Ci sono tantissimi altri strumenti e modalità con cui potrei fare queste ed altre operazioni di esplorazione e analisi, ma questo è un *post* dedicato in modo particolare a DuckDB e alla sua applicazione a riga di comando. Quindi **da qui in poi**, utilizzerò **soprattutto** **DuckDB** e la sua **`cli`**.<br>
...
È una tabella di gran valore, perché consente già di prendere delle scelte di analisi e di trasformazione dei dati e avere restituito degli elementi di qualità dei dati.
...
Anche con 50.000 righe, ho errori di inferenza. Stavolta è per il campo `COD_TIPO_PROCED_ATTIVAZIONE`, che di base sembra contenere numeri, ma alle volte contiene il carattere `.`.<br>
...
Portando il numero di righe da leggere a 100.000, non ho più errori. Ed è molto comodo - per un file "grande", non ottimizzato per le *performance* e compresso - ottenere l'*output* di `SUMMARIZE` in circa 10 secondi.<br>
...
Tra i campi categorici `OC_MACROAREA`, che definisce le macro aree geografiche italiane. E per sapere ad esempio qual è il finanziamento pubblico totale per ciascuna, si può usare la query `SQL`

```{#lst-query-macroarea .sql lst-cap="Query SQL: totale dei finanziamenti pubblici per macro aree"}
SELECT OC_MACROAREA,
...
che restituirà qualcosa come in @tbl-aree-sum
...
- [**trasformare** il file **`CSV`** di input in un file **`PARQUET`**](#convertire-il-file-di-opencoeseione-in-parquet) e lavorare su questo.
...
- `COD_TIPO_PROCED_ATTIVAZIONE` è dichiarato come num, ma è una stringa, perché contiene.
...
- encoding, caratteri errati
- qual è lo schema di definizione dei campi? Perché solo num e nessuna distinzione tra interi e float?

## In conclusione

- comprimere i file csv grande, con gzip
- pubblicare CSV standard
- descrivere i CSV
- quanto è figo duckdb
- il parquet come futuro formato open data
- usare SQL
- non voglio che vinca il CSV, ma è comodo e finché dura

## Da sviluppare

- [ ] verificare data di pubblicazione di questo post quarto
- [ ] rivedere introduzione, sottolineando che non voglio diffondere l'uso del CSV
- [x] parquet al posto dei CSV https://www-icem7-fr.translate.goog/outils/parquet-devrait-remplacer-le-format-csv/?_x_tr_sl=auto&_x_tr_tl=it&_x_tr_hl=en-US
- [x] allegare sempre tracciato record machine readable
- [x] il SQL è standard, vecchio e diffuso
- [x] comprimere i CSV
- [x] il fatto che il csv sia brutto all'inizio
- [x] il fatto che non voglio che il mondo sia inondato di CSV
- [x] pubblicare CSV standardizzati (utf-8, decimali, field separator, uniformità campi,date)
- [x] capitolo frictionless e schema SQL
- [x] fare cenno alla compressione ZSTD
- [x] le estensioni di duckdb

## Spunti

- [x] dati spaziali download https://github.com/OvertureMaps/data/blob/main/README.md#3-duckdb-sql
- [ ] tldr
- [ ] https://kestra.io/blogs/2023-08-11-dataframes
- [ ] dbeaver
- [ ] spunti https://medium.com/walmartglobaltech/duckdb-vs-the-titans-spark-elasticsearch-mongodb-a-comparative-study-in-performance-and-cost-5366b27d5aaa
