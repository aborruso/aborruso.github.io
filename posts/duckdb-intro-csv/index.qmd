...
In questo post si parla dei dati in formato CSV di **OpenCoesione**. Ha suscitato il loro interesse e ci siamo confrontati sull'opportunità di pubblicare i loro dati anche in  21 febbraio 2024.<br>
🏅 Ebbene, dal 22 febbraio 2024 **hanno pubblicato i loro dati in questo formato**. I dettagli e un lungo tutorial in [questo post](../leggere-interrogare-file-parquet/index.qmd).
:::
...
In questo lungo post farò una carrellata della [**compressione** dei file **`CSV`**](#compressione-dei-file-csv), dell'[importanza di **descriverli**](#descrivere-i-csv), dell'utilizzo di [**DuckDB**](#duckdb) per analizzarli e del [**formato Parquet**](#parquet) come alternativa al CSV.
...
L'ultimo esempio, per chiudere in bellezza, è quello dell'impareggiabile [**portale Open Data della Francia**](https://www.data.gouv.fr/).<br>
Pubblica un dataset in formato `parquet`, sono disponibili gli URL statici dei file, ed è possibile fare *query* in modo comodissimo, come se fossero [disponibili delle API](#come-avere-delle-api).<br>
Ad esempio posso interrogare il [file da 470 MB degli indirizzi](https://www.data.gouv.fr/fr/datasets/bureaux-de-vote-et-adresses-de-leurs-electeurs/), di 15 milioni di righe, per avere il numero di indirizzi per comune con questa semplicità (vedi @lst-query-francia):

```{#lst-query-francia .bash lst-cap="Esempio di *query* da *shell*"}
duckdb -c '
load httpfs;
...
E in pochi secondi si ha restituito il risultato (vedi @fig-portale-opendata-francia).

![Output da shell](images/portale-opendata-francia.png){#fig-portale-opendata-francia width=100%}
...
- [**Data Elixir - Issue 450**](https://newsletterest.com/message/185579/Data-Elixir-Issue-450);
- [**532: quantum of sollazzo**](https://buttondown.email/puntofisso/archive/532-quantum-of-sollazzo/), di Giuseppe Sollazzo;
- [**LaCulturaDelDato #094**](https://stefanogatti.substack.com/p/laculturadeldato-094#LaCulturaDelDato), di Stefano Gatti.
:::
